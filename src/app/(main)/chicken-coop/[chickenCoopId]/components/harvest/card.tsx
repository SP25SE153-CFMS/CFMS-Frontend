...
export default function CardHarvest({ chickenCoopId }: { chickenCoopId: string }) {
    const currentHarvestLogs = harvestLogs.filter(
        (harvest) => harvest.chickenCoopId === chickenCoopId,
    );
    ...
    return (
        <Card className="p-6 mb-4">
            <div className="flex flex-wrap items-center justify-between gap-x-4 space-y-2">
                ...
                <div className="flex gap-2">
                    <Button
                        variant="outline"
                        className="space-x-1"
                        onClick={() => downloadCSV(currentHarvestLogs, 'harvest-logs.csv')}
                    >
            ...
            <div className="-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-x-12 lg:space-y-0">
                <DataTable data={currentHarvestLogs} columns={columns} />
            </div>
        </Card>
    );
}
